...
Diffusion models have recently emerged as one of the most powerful classes of deep generative models, achieving strong performance across a wide range of domains. They construct a forward process that gradually corrupts data with Gaussian noise and train a neural network to approximate the reverse-time dynamics that iteratively remove noise. This yields a generative process that is both probabilistically principled and empirically effective, offering advantages in sample quality and training stability over earlier paradigms such as variational autoencoders (VAEs) \citep{kingma2022AutoEncodingVariationalBayes} and generative adversarial networks (GANs) \citep{goodfellow2020GenerativeAdversarialNetworks}. Their practical impact has been most visible in image synthesis: denoising diffusion probabilistic models (DDPMs) \citep{hoDenoisingDiffusionProbabilistic2020} showed that diffusion could match or surpass GANs in fidelity and diversity, while advances such as improved variance schedules and classifier guidance \citep{dhariwal2021DiffusionModelsBeat} established diffusion as the state-of-the-art for high-resolution image generation. More recently, large-scale systems such as Imagen \citep{saharia2022PhotorealisticTexttoImageDiffusion} and Stable Diffusion \citep{rombach2022HighResolutionImageSynthesis} have extended this influence to text-to-image generation, where diffusion models now dominate both research and industry applications.

Score-based diffusion models \citep{song2019GenerativeModelingEstimating,songImprovedTechniquesTraining2020,song2021ScoreBasedGenerativeModeling,songMaximumLikelihoodTraining2021} extend denoising diffusion models by shifting the focus from directly learning the reverse process to estimating the score function, i.e.\ the gradient of the log-density of the noisy distribution. This reformulation naturally generalises the framework to continuous time via stochastic differential equations (SDEs), enabling different forward noise processes and corresponding reverse-time samplers to be employed. Beyond images, score-based diffusion has been successfully applied to diverse data modalities, including audio synthesis \citep{kong2021DiffWaveVersatileDiffusion}, 3D point cloud generation \citep{luo2021DiffusionProbabilisticModels}, and molecular conformations \citep{pmlr-v162-hoogeboom22a}. These successes illustrate the flexibility of the diffusion framework and motivate its application to more structured scientific domains such as protein modelling.

\subsection{Motivation for Protein Structure Generation}
Proteins are essential macromolecules whose biological functions are determined by their three-dimensional structures. Even subtle variations in structure can dramatically affect stability, dynamics, and interactions with other molecules \citep{creighton1993proteins,branden2012IntroductionProteinStructure}. The ability to generate novel protein backbones therefore holds great promise for biomedical science, with potential applications in de novo protein design, drug discovery, and enzyme engineering. Unlike small molecules, whose chemical space can be systematically enumerated, proteins inhabit a vast and highly constrained landscape, making data-driven generative modelling an appealing but challenging strategy.

Protein structures are subject to strict physical and geometric constraints. Bond lengths and bond angles are restricted to narrow ranges, torsional angles follow characteristic empirical distributions, and steric effects prevent atoms from occupying unrealistic positions \citep{creighton1993proteins,branden2012IntroductionProteinStructure}. Beyond these local constraints, proteins must also adopt globally compact and physically plausible folds in order to be stable. These factors make the generative task significantly more challenging than in domains such as images or text, where the space of valid configurations is much less restricted.

Deep learning has already transformed structural biology, most prominently through advances in structure prediction. AlphaFold \citep{jumper2021HighlyAccurateProtein}, for example, demonstrated the power of data-driven models for predicting native protein structures from amino acid sequences with unprecedented accuracy. However, prediction and generation are fundamentally distinct: while prediction maps known sequences to their most likely structures, generative modelling seeks to explore the broader landscape of possible folds, independent of sequence. The problem of generating realistic and diverse protein backbones thus remains comparatively underexplored, yet it represents a key step toward enabling computational design of entirely new proteins with tailored functions.
...
\begin{itemize}
    ...
    \item \textbf{Variance-exploding (VE) SDE}: variance grows unbounded as \(t \to 1\), pushing the distribution to white noise.  
...
The Predictor-Corrector (PC) sampler, introduced by \citet{song2021ScoreBasedGenerativeModeling}, improves reverse-time SDE sampling by combining numerical integration with score-based MCMC refinement. The method alternates between two complementary updates:
\begin{enumerate}
    \item \textbf{Predictor step.} A numerical update of the reverse SDE, for example using Euler-Maruyama (\cref{eq:Euler-Maruyama-reverse-SDE}), which advances the sample trajectory toward lower noise levels.
    \item \textbf{Corrector step.} One or more stochastic refinement steps based on Langevin Monte Carlo \citep{parisi1981CorrelationFunctionsComputer}, which adjust the sample distribution to better match the target marginal:
        \begin{align*}
            x \;\leftarrow\; x + \epsilon\, s_\theta(x,t) + \sqrt{2\epsilon}\,z, 
            \quad z \sim \mathcal{N}(0,I),
        \end{align*}
        where \(\epsilon\) is a step size. This correction nudges samples toward regions of higher likelihood under the learned score.
\end{enumerate}
This alternating scheme mirrors classical predictor-corrector methods in numerical analysis \citep{allgower2012numerical}, with the predictor providing a coarse trajectory update and the corrector refining local accuracy. By alternating predictor and corrector steps, the algorithm achieves a better trade-off between quality and efficiency than pure reverse SDE sampling. In practice, only a small number of corrector iterations per step are sufficient to noticeably improve sample fidelity while retaining efficiency \citep{song2021ScoreBasedGenerativeModeling}.
...
\begin{remark}[Scope and interpretation.]
    Absolute scores in this study should be interpreted in light of the modelling scope: we target backbone-only C\(_\alpha\) generation with modest model sizes, and we evaluate by pairing each sample with its specific native reference. As mentioned, high RMSD or low TM-score does not preclude physical plausibility—it indicates divergence from the particular native fold. Within this paired-evaluation setting, the observed improvements from positional encoding, residual design, and PC sampling are consistent and robust across the dataset splits considered.
\end{remark}
...
The experiments in \cref{sec:Experiments} show that score-based diffusion models can generate protein backbones with meaningful similarity to native folds. Across all architectures, the Transformer-based GCN with positional embeddings consistently achieved the best RMSD and TM-scores. This highlights that sequential order, encoded through sinusoidal embeddings of amino acid indices, is an essential complement to geometric edge features such as distances and orientations. Without positional cues, the GCN struggled to recover global fold organisation, underscoring the importance of integrating both spatial and sequential information.

Distributional analyses provide further insight. Box and density plots reveal that the positional GCN not only improves mean performance but also reduces variability, shifting the bulk of the distribution toward lower RMSD and higher TM-scores. By contrast, the UNet baseline produces reasonable central values but exhibits broader spread, reflecting instability introduced by zero-padding. The GCN without positional encoding performs worst, with many collapsed folds, indicating that purely geometric neighbourhoods are insufficient for stable generation. Backbone length also plays a role: RMSD increases with \(L\) as local deviations accumulate, while TM-scores decline more gradually. The positional GCN shows greater robustness across long chains, suggesting improved propagation of long-range information.

Finally, qualitative PyMOL overlays complement these findings. The positional GCN generally preserves global topology with moderate local errors, the UNet captures local backbone patterns but suffers from padding artefacts, and the GCN without positional encoding fails to maintain fold structure. While individual visualisations are not sufficient for strong conclusions, they align with quantitative trends and provide intuitive evidence of how architectural choices affect structural plausibility. Together, these results demonstrate that score-based diffusion can successfully model protein backbones, but that design decisions such as positional embeddings critically influence fidelity.

\subsection{Limitations}\label{subsec:limitations}
While the results are encouraging, several limitations must be acknowledged.
\paragraph{Geometric constraints.}  
No explicit bond length or bond angle constraints were imposed during training or sampling. The model instead learned approximate geometry directly from data, which generally produced chemically reasonable backbones but does not guarantee stereochemical validity. In particular, without enforcing hard constraints, subtle violations such as distorted bond angles or unrealistic torsions may occur. Integrating explicit geometric constraints, as explored in other generative frameworks \citep{watsonNovoDesignProtein2023,yimDiffusionModelsProtein2024,wuProteinStructureGeneration2024}, would ensure physical plausibility in every generated sample.

\paragraph{Backbone-only representation.}  
This study focused exclusively on C\(_\alpha\)-only representations, ignoring side chains and residue types. While this simplifies the state space and enables coarse-grained modelling of global folds, it limits biological interpretability. For instance, side chains play a central role in protein stability, interactions, and functional specificity \citep{creighton1993proteins,branden2012IntroductionProteinStructure}. Without them, generated structures cannot be directly evaluated in terms of biochemical function, and the framework cannot yet support sequence-conditioned protein design.

\paragraph{Model scale.}  
The models employed here are modest in size compared to state-of-the-art diffusion models in protein design \citep{wuProteinStructureGeneration2024,watsonNovoDesignProtein2023}. Despite this, they successfully generated backbones of variable lengths, including chains exceeding 1000 residues, demonstrating the feasibility of the approach. Nonetheless, scaling to larger architectures—particularly those incorporating SE(3)-equivariance, global attention, or pretraining on large structural databases—would likely improve performance and stability, especially for long proteins with complex folds.

\paragraph{Evaluation metrics.}  
As discussed in \cref{subsec:metrics}, RMSD and TM-score provide widely used measures of structural similarity but capture fidelity to a particular reference fold rather than unconditional structural validity. Our evaluation reflects reconstruction accuracy in a paired setting rather than the broader question of whether generated backbones are universally valid. Future work could incorporate additional metrics or physical simulations to assess chemical realism and functional plausibility.

\subsection{Related Work and Future Directions}\label{subsec:future-work}
Recent years have seen rapid progress in generative modelling of protein structures. Diffusion-based methods in particular have achieved state-of-the-art results: \citet{watsonNovoDesignProtein2023} demonstrated that RoseTTAFold-based diffusion models can generate novel proteins with designed functions, while FoldingDiff \citep{wuProteinStructureGeneration2024} applied score-based diffusion directly to backbone coordinates. At the architectural level, SE(3)-equivariant neural networks \citep{satorras2021EquivariantGraphNeural,geiger2022E3nnEuclideanNeural} have been proposed to improve geometric fidelity and long-range modelling, building on earlier work in tensor field networks and steerable CNNs \citep{thomasTensorFieldNetworks2018,weiler3DSteerableCNNs2018,kondorClebschGordanNetsFully2018}. These developments illustrate both the rapid maturation of diffusion-based approaches and the growing emphasis on embedding physical symmetries into generative models. Building on these ideas, and motivated by the limitations identified in this dissertation, several promising directions follow.

\paragraph{Constraint integration.}  
One natural extension is to incorporate stereochemical constraints directly into the generative process. In this study, bond lengths, bond angles, and torsional distributions were only learned approximately from data, which can lead to subtle deviations from physical validity. Future models could embed these constraints within the diffusion dynamics, ensuring that generated samples remain on a chemically valid manifold throughout the generative trajectory. Recent advances in Riemannian score-based diffusion \citep{debortoliRiemannianScoreBasedGenerative2022,huang2022RiemannianDiffusionModels,chungImprovingDiffusionModels2022} provide a principled framework for defining SDEs and training objectives on constrained spaces, and could be adapted to enforce protein backbone geometry more rigorously.

\paragraph{Advanced architectures.}  
Another promising direction is the adoption of architectures with built-in geometric equivariance. While the graph neural networks employed here capture local geometry via edge features, they are not strictly equivariant under global rotations or reflections. SE(3)-equivariant GNNs \citep{thomasTensorFieldNetworks2018,weiler3DSteerableCNNs2018,kondorClebschGordanNetsFully2018,geiger2022E3nnEuclideanNeural,geiger2025EuclideanNeuralNetworks} overcome this limitation by construction, ensuring that predictions transform consistently with input coordinates. This property is particularly relevant for protein backbones, where structural validity should be invariant to overall orientation. Recent works such as \citet{yimDiffusionModelsProtein2024} have begun exploring this idea in the context of protein diffusion models, suggesting that equivariant architectures may further improve both stability and fidelity.

\subsection{Conclusion}\label{subsec:conclusion}
This dissertation has explored the application of score-based diffusion models to the task of protein backbone generation. We first reviewed the theoretical foundations of diffusion modelling, then developed a graph-based representation of protein backbones tailored to C\(_\alpha\) geometry, and implemented several neural architectures for score estimation. Experiments on the CATH S40 dataset demonstrated that a residual Transformer-based GCN with sinusoidal positional embeddings outperformed both a UNet baseline and a graph model without positional information.

The results indicate that diffusion models combined with graph-based architectures provide a promising approach to coarse-grained protein structure generation. While the models developed here are modest in scale and focus only on backbones, the study highlights the importance of sequential information in graph representations and points toward clear avenues for future work, including the integration of stereochemical constraints, scaling to larger and more expressive architectures, and extending the framework to sequence-conditioned generation.

\clearpage

\appendix
\section{Code Appendix}
This appendix provides excerpts from the implementation of the graph neural network (GNN) model described in the main text (\cref{subsubsec:Transformer-GCN}). The code is written in Python and makes use of standard machine learning libraries, including PyTorch and PyTorch Geometric (PyG) \citep{PyG1.0,PyG2.0}. For readability, only the core components of the model are shown here. Note that this is a code intensive project, the complete source code used in this dissertation, including data preprocessing, training and sampling scripts, and relavant utilities, is available in the accompanying GitHub repository: \url{https://github.com/Yuanhao-JIANG/protein-backbone-diffusion}.
