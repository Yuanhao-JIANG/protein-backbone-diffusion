...
For each time \(t\), the score function of \(x_t\) can be trained via minimising the Fisher divergence between the parameterised model and the true score
\begin{align}\label{eq:FD}
    J_t\left(\theta\right)=\mathbb{E}_{p\left(x_t\right)}\left[\norm{s_{\theta}\left(x_t, t\right)-\nabla_{x_t}\log p\left(x_t\right)}_2^2\right].
\end{align}
The unknown term, true score, \(\nabla_{x_t}\log p\left(x_t\right)\) can be eliminated with the score matching objective \cite{hyvarinenEstimationNonNormalizedStatistical2005}
\begin{align*}
...
\end{align*}
However, the term \(\tr\left(\nabla_{x_t}s_{\theta}\left(x_t,t\right)\right)\) can be computationally intensive, especially when the data is in a high dimensional space. \citet{vincentConnectionScoreMatching2011} showed that training a denoising autoencoder to reconstruct clean data from corrupted observations is equivalent to score matching under certain conditions. Indeed, decomposing \cref{eq:FD} yields
\begin{flalign}
    \nonumber
    J_t\left(\theta\right)
    &=\mathrlap{\mathbb{E}_{p\left(x_t\right)}\left[\norm{s_{\theta}\left(x_t, t\right)-\nabla_{x_t}\log p\left(x_t\right)}_2^2\right]}\\
    \nonumber
    &=\mathrlap{\mathbb{E}_{p\left(x_t\right)}\left[\norm{s_{\theta}\left(x_t,t\right)}_2^2\right]-2\mathbb{E}_{p\left(x_t\right)}\left[s_{\theta}^T\left(x_t,t\right)\nabla_{x_t}\log p\left(x_t\right)\right]}\\
    \label{eq:DSM-derivation-0}
    & &&& +\mathbb{E}_{p\left(x_t\right)}\left[\norm{\nabla_{x_t}\log p\left(x_t\right)}_2^2\right]
\end{flalign}
where \(C\coloneq\mathbb{E}_{p\left(x_t\right)}\left[\norm{\nabla_{x_t}\log p\left(x_t\right)}_2^2\right]\) is independent of \(\theta\), and now by conditioning on clean data,
\begin{align}
